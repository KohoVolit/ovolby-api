...
def county2province(county):
    if county == '9999':
        return '3100'               #jihocesky (2006) !!
    if county == '1100':
        return '1000'       #praha
    if int(county) < 1200 and int(county) > 1100:
        return '1000'
    return county[0:2] + '00'

for row in sums:
    election_id = 'psp.cz-'+county2province(row[3])+'-2006'
    result = {
...
# COUNTS
vts = []
with open(path + "PST4p.csv") as fd:
    csvr = csv.reader(fd)
    i = 0
    for row in csvr:
        if i > 0:
            vts.append(row)
        i = i + 1
vts = sorted(vts,key=operator.itemgetter(4,5))
